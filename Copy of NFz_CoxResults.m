...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
do_print = true;
do_anti_corrs = false; %look at anti-correlations instead
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if do_anti_corrs
    corr_str = 'anti-correlated'; % for null result print out
else
    corr_str = 'positively correlated'; % for null result print out
end
    
disp('**** START Flags ****');
disp([num2str(do_print),' - do_print',10,...
    num2str(do_anti_corrs),' - do_anti_corrs']);
disp('**** END Flags ****');
disp(sprintf('\n'));
...
for i=1:length(toxicities)
    ...
    for j=1:length(structures)
        ...
        %% load data
        fn = ['NFZ_',structures{j},'_',toxicities{i},'_data.mat'];
        disp(['']);
        disp(['Loading ',fn]);
        disp(['']);
        ...
        if ~no_corr,
            ...
            h_mx_logl=plot([x_dvx(doseloc) x_dvx(doseloc)],ylim,'b--','LineWidth',2);
            xlim([0 max(x_dvx)]);
            set(gca,'fontsize',18);
            ...
            logl_str = {'Positive Corr.',...
                ...
                '68% CL'};
            legend([h_corr h_acorr h_mx_logl h_low_68cl],logl_str,'Location','Best');
        else  %still printing empty figure for slides
            hold on;
            set(gca,'fontsize',18);
            set(gca,'xminortick','on','yminortick','on');
            set(gca,'box','on');
            xlim([0 max([CGobj.mBinsVol])]);
            xlabel('(D_{V}) Volume [cc]','fontsize',20);
            ylabel('Cox model log-likelihood','fontsize',20);
            text(0.5,0.5,['No ',corr_str,10,'Cox models found'],...
                'Units','normalized',...
...
%         end
%         
%         no_corr = (sum(flgCox)<2); % no correlations found        
%     
%     
%         DVxCox = DVxCox(flgCox);
%         x_dvx=CGobj.mBinsVol(flgCox);
%         
%         
%         %% DVx llhds
%         logl = [DVxCox.logl]'; %logl(~flgCox) = -inf; % log likelihood of Cox model, anti-correlation points not counted
...
%         
%         if ~no_corr,
%             plot(x_dvx, [DVxCox.logl],'o','MarkerSize',20,'LineWidth',2);hold on;
%             h_low_68cl=plot([0 max(x_dvx)],[lowCI68 lowCI68],'r--');
...
%             xlabel('(D_{V}) Volume [cc]','fontsize',20);
%             ylabel('Cox model log-likelihood','fontsize',20);
%             logl_str = {['Max LogL = ', num2str(mx,3),10,...
%                 'at D_{',num2str(x_dvx(doseloc),3),' cc}'],...
%                 '68% CL'};
%             cur_lgnd=legend([h_mx_logl h_low_68cl],logl_str,'Location','Best');
...
%         end
        ...
        if ~no_corr, %still print empty figure for slides
            plot(x_dvx,p','k-','LineWidth',1);hold on;
            h_pos_pval=semilogy(x_dvx(flgCox),p(flgCox)','.','MarkerSize',20);hold on;
            h_neg_pval=semilogy(x_dvx(~flgCox),p(~flgCox)','r.','MarkerSize',20);
            h_sig=semilogy([0 max(x_dvx)],[0.05 0.05],'g--');
            h_min_pval = semilogy([x_pos_dvx(ploc) x_pos_dvx(ploc)],ylim,'--','LineWidth',2);
            hold off;
            xlim([0 max(x_dvx)]);
            ylim([min(min(p),0.05)-0.01 1]);
            set(gca,'fontsize',18);
            set(gca,'xminortick','on','yminortick','on');
            set(gca,'box','on');
            pval_str = {'Positive Corr.',...
                ...
                'p = 0.05'};
            cur_lgnd=legend([h_pos_pval h_neg_pval h_min_pval h_sig],pval_str,'Location','Best');
            %set(cur_lgnd,'FontSize',20);
            xlabel('(D_{V}) Volume [cc]','fontsize',20);
        ...
        %% VDx llhds
        cur_fig=figure(cur_fig_ctr+400); clf reset;
        set(gcf,'Position',ss_four2three);
        if ~no_corr, 
            h_pos_vdx=plot(x_vdx, [VDxCox.logl],'.-','MarkerSize',20,'LineWidth',2);
            hold on;
            h_vdx_68ci=plot([0 max(x_vdx)],[lowCI68 lowCI68],'r--');
            h_mx_logl=plot([x_vdx(doseloc) x_vdx(doseloc)],ylim,'g--','LineWidth',2);
            xlim([0 max(x_vdx)]);
            set(gca,'fontsize',18);
            set(gca,'xminortick','on','yminortick','on');
            set(gca,'box','on');
            ...
            ylabel('Cox model log-likelihood','fontsize',20);
            logl_str = {['Max LogL = ', num2str(mx,3),10,...
                'at V_{',num2str(x_vdx(doseloc),3),' Gy}'],...
                '68% CL'};
            
            cur_lgnd=legend([h_mx_logl h_vdx_68ci],...
                logl_str,'Location','Best');
            set(cur_lgnd,'FontSize',20)
        else  %still printing empty figure for slides
        ...
        if ~no_corr,
            ...
            semilogy(x_vdx,[VDxCox.p],'.-','MarkerSize',20,'LineWidth',2);
            hold on;
            h_sig=semilogy([0 max(x_vdx)],[0.05 0.05],'r--');
            h_min_pval = semilogy([x_vdx(ploc) x_vdx(ploc)],ylim,'g--','LineWidth',2);
            hold off;
            xlim([0 max(x_vdx)]);
            ylim([min(min_p,0.05)-0.01 1]);
            set(gca,'fontsize',18);
            set(gca,'xminortick','on','yminortick','on');
            set(gca,'box','on');
            pval_str = {['Min p-val = ', num2str(min_p,2),10,...
                'at V_{',num2str(x_vdx(ploc),3),' cc}'],...
                'p = 0.05'};
            cur_lgnd=legend([h_min_pval h_sig],pval_str,'Location','Best');
            set(cur_lgnd,'FontSize',20);
            xlabel('(V_{D}) Dose [Gy]','fontsize',20);
            ylabel('Cox model p-value','fontsize',20);
        
        else %still print empty figure for slides
...
%     set(gca,'xminortick','on','yminortick','on');
%     set(gca,'box','on');
%     pval_str = {['Min p-val = ', num2str(min_p,3),' at ',num2str(x_dvx(ploc),4),' cc']};
%     legend(h_min_pval,pval_str,'Location','SouthEast');
...
%     x_dvx=CGobj.mBinsDose(flgCox);
%
%     % logl
%     figure(fig_num); clf reset;
%     plot(x_dvx, [VDxCox.logl],'.-');
%     fig_num=fig_num+1;
%     set(gcf,'Position',ss_four2three);
%     hold on;
%     plot(x_dvx,repmat(lowCI68,size(x_dvx)),'r--');
%     plot(x_dvx,repmat(lowCI95,size(x_dvx)),'c--');
%     h_mx_logl=plot([x_dvx(doseloc) x_dvx(doseloc)],ylim,'g--','LineWidth',2);
%     xlim([0 max(x_dvx)]);
%     set(gca,'fontsize',14);
%     set(gca,'xminortick','on','yminortick','on');
...
%     logl_str = {['Max LogL = ', num2str(mx,5),' at ',num2str(x_dvx(doseloc),4),' cc']};
%     legend(h_mx_logl,logl_str,'Location','Best');
%
%
%     % p-vals
%     p = ones(size(VDxCox));
...
%     set(gca,'box','on');
%         pval_str = {['Min p-val = ', num2str(min_p,3),' at ',num2str(x_dvx(ploc),4),' cc']};
%     legend(h_min_pval,pval_str,'Location','Best');
...
%
%     % V42 + KPS
%     [kpsCox,~,~] = CGobj.fCoxParameter_DVH('KPS');
...
%      %% V_{39} + Tx
%
%     %[biCox,flgBiCox,flgBiAnti] = CGobj.fCoxParameter_DVH('VDxTx');
%     f = cellfun(@(x) strcmpi('VDxTx',x),CGobj.mCoxParameter(:,1)); % search the label
%     biCox = CGobj.mCoxParameter{f,2}; % extract Cox model result
...
%     %% V_{39} + cm2cw
%
%     f = cellfun(@(x) strcmpi('VDxCM2CW',x),CGobj.mCoxParameter(:,1)); % search the label
%     biCox = CGobj.mCoxParameter{f,2}; % extract Cox model result
%     flgCox = ~arrayfun( @(y) any(structfun(@(x) any(isempty(x(:)))|any(isinf(x(:))), y)), biCox); % some fields are empty or infinite, indicating no data for those values
%     tmp_beta = {biCox.beta};
%     is_inf = cellfun(@(x) length(x),tmp_beta);
...
%      %% V_{39} + BMI
%
%     %[biCox,flgBiCox,flgBiAnti] = CGobj.fCoxParameter_DVH('VDxBMI');
...
%
%     %[biCox,flgBiCox,flgBiAnti] = CGobj.fCoxParameter_DVH('VDxBMI');
%     f = cellfun(@(x) strcmpi('VDxDperFx',x),CGobj.mCoxParameter(:,1)); % search the label
%     biCox = CGobj.mCoxParameter{f,2}; % extract Cox model result
%     flgCox = ~arrayfun( @(y) any(structfun(@(x) any(isempty(x(:)))|any(isinf(x(:))), y)), biCox); % some fields are empty or infinite, indicating no data for those values
%     tmp_beta = {biCox.beta};
%     is_inf = cellfun(@(x) length(x),tmp_beta);
...
%
%     [biCox,flgBiCox,flgBiAnti] = CGobj.fCoxParameter_DVH('DVxFx');
%     flgBiCox(flgBiAnti(:,1))=false; % anti-correlations were not be considered
%
%     [uniCox,flgUniCox,flgUniAnti] = CGobj.fCoxParameter_DVH('DVx');
%     flgUniCox(flgUniAnti)=false; % anti-correlations were not be considered
%
%     %flgUniBiCox=flgUniCox&flgBiCox;
%     flgUniBiCox=flgUniCox;
%
%     volbins = CGobj.mBinsVol(flgUniBiCox);
%
%     % plot log-likelihood
%     uni_logl = [uniCox(flgUniBiCox).logl];
%     bi_logl = [biCox(flgUniBiCox).logl];
...
%
%     f = cellfun(@(x) strcmpi('DVxTx',x),CGobj.mCoxParameter(:,1)); % search the label
%     biCox = CGobj.mCoxParameter{f,2}; % extract Cox model result
%     flgCox = ~arrayfun( @(y) any(structfun(@(x) any(isempty(x(:)))|any(isinf(x(:))), y)), biCox); % some fields are empty or infinite, indicating no data for those values
%     tmp_beta = {biCox.beta};
%     is_inf = cellfun(@(x) length(x),tmp_beta);
...
%
%     f = cellfun(@(x) strcmpi('DVxCM2CW',x),CGobj.mCoxParameter(:,1)); % search the label
%     biCox = CGobj.mCoxParameter{f,2}; % extract Cox model result
%     flgCox = ~arrayfun( @(y) any(structfun(@(x) any(isempty(x(:)))|any(isinf(x(:))), y)), biCox); % some fields are empty or infinite, indicating no data for those values
%     tmp_beta = {biCox.beta};
%     is_inf = cellfun(@(x) length(x),tmp_beta);
...
%         %% DV + BMI
%
%     f = cellfun(@(x) strcmpi('DVxBMI',x),CGobj.mCoxParameter(:,1)); % search the label
%     biCox = CGobj.mCoxParameter{f,2}; % extract Cox model result
%     flgCox = ~arrayfun( @(y) any(structfun(@(x) any(isempty(x(:)))|any(isinf(x(:))), y)), biCox); % some fields are empty or infinite, indicating no data for those values
%     tmp_beta = {biCox.beta};
%     is_inf = cellfun(@(x) length(x),tmp_beta);
...
%     %% DV + Dose/Fraction
%
%     f = cellfun(@(x) strcmpi('DVxDperFx',x),CGobj.mCoxParameter(:,1)); % search the label
%     biCox = CGobj.mCoxParameter{f,2}; % extract Cox model result
%     flgCox = ~arrayfun( @(y) any(structfun(@(x) any(isempty(x(:)))|any(isinf(x(:))), y)), biCox); % some fields are empty or infinite, indicating no data for those values
%     tmp_beta = {biCox.beta};
%     is_inf = cellfun(@(x) length(x),tmp_beta);
