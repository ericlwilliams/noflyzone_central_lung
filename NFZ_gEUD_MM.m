...
structures = {'ESOPHAGUS' 'HEART' 'ILUNG'   'NFZ' 'PBT' 'LUNGS'};
...
for i=1:length(toxicities)
    ...
    %% loglikelihoods
    figure(i+10000); clf reset;
    set(gcf,'Position',ss_four2three);
    
      %% t-stats
    figure(i+100000); clf reset;
    set(gcf,'Position',ss_four2three);
    
       %% t-stats
    figure(i+1000000); clf reset;
    set(gcf,'Position',ss_four2three);
    ...
    h_pvals = zeros(length(structures),1);
    min_lr_pvals = zeros(length(structures),1);
            h_tvals = inf(length(structures),1);

    h_mm_tstats = zeros(length(structures),1);
    h_mm_pvals = zeros(length(structures),1);
    min_mm_lr_pvals = zeros(length(structures),1);
    t_stats = cell(length(structures),1);% each cell is list of t-stats for each n value
    for j=1:length(structures)
        ...
        [h_mm_pvals(j), min_mm_lr_pvals(j),t_stats{j}]=CGobj.fLogisticRegressionPvalueExactFig_MM_a_EUD(strcat(str_colors{j},'s--'),2);
       
        figure(i+100000);hold on;
        h_mm_tstats(j)=plot(-LymanN,t_stats{j},strcat(str_colors{j},'s--'),'LineWidth',2);
        
        figure(i+1000000);hold on;
        ...
        cur_fig=figure(i+1000); clf reset;
        set(gcf,'Position',ss_four2three);
        
        h_mle=CGobj.fLogisticRegressionGridExactFig_a_loglikelhood_EUD('rs--',2);
        %h_mm=CGobj.fLogisticRegressionGridExactFig_a_loglikelhood_EUD('bs--',2,'MM');
        %lgnd_eud_logl= legend([h_mle h_mm], 'Maximum Likelihood','Mixture Model','Location','Best');
        lgnd_eud_logl= legend(h_mle, 'Maximum Likelihood','Location','Best');
        set(lgnd_eud_logl,'FontSize',18);
        set(gca,'xminortick','on','yminortick','on');
        set(gca,'box','on');
        set(gca,'fontsize',16);
        xlabel('log_1_0(a)','fontsize',20);
        ylabel('log-likelihood per degreee of freedom','fontsize',20);
        if do_print,
            set(cur_fig,'Color','w');
            export_fig(cur_fig,...
                [fig_basename,'_mle_logl'],'-pdf');
            disp(['Saving ',fig_basename,'_mle_logl.pdf']);
            
        end
        
        cur_fig=figure(i+2000); clf reset;
        set(gcf,'Position',ss_four2three);
        
        %h_mle=CGobj.fLogisticRegressionGridExactFig_a_loglikelhood_EUD('rs--',2);
        h_mm=CGobj.fLogisticRegressionGridExactFig_a_loglikelhood_EUD('bs--',2,'MM');
        lgnd_eud_logl= legend(h_mm, 'Mixture Model','Location','Best');
        set(lgnd_eud_logl,'FontSize',18);
        set(gca,'xminortick','on','yminortick','on');
        set(gca,'box','on');
        set(gca,'fontsize',16);
        ...
        if do_print,
            set(cur_fig,'Color','w');
            export_fig(cur_fig,...
                [fig_basename,'_mm_logl'],'-pdf');
            disp(['Saving ',fig_basename,'_mm_logl.pdf']);
        ...
        [loga,~,h_resp_mle] = CGobj.fLogisticRegressionGridRespondingCurveExactFig_a_EUD(0,'r',1);
        [~,pval,h_resp_mm] = CGobj.fLogisticRegressionGridRespondingCurveExactFig_a_EUD(0,'b',1,'MM');
        ...
        lgnd_eud_logl_dmean= legend([h_resp_mle h_resp_mm], 'Maximum Likelihood','Mixture Model','Location','Best');
        set(lgnd_eud_logl_dmean,'FontSize',16);
        ...
        [loga,pval,h_resp_mle] = CGobj.fLogisticRegressionGridRespondingCurveExactFig_a_EUD('loga','r',1);
        [~,pval,h_resp_mm] = CGobj.fLogisticRegressionGridRespondingCurveExactFig_a_EUD('loga','b',1,'MM');
        %[loga,pval] = CGobj.fLogisticRegressionRespondingCurveExactFig_a_EUD('loga','k',1);
        %[loga,pval] = CGobj.fLogisticRegressionRespondingCurveExactFig_a_EUD(1,'k',1);
        CGobj.fComplicationObservedFig_EUD(loga,4,'k*',2);
        ...
        str = sprintf(loga_str,num2str(loga),num2str(pval,'%3.2e'));
        text(0.65,0.85,str,'FontSize',24,'Units','normalized');
        lgnd_eud_logl_best= legend([h_resp_mle h_resp_mm], 'Maximum Likelihood','Mixture Model','Location','Best');
        set(lgnd_eud_logl_best,'FontSize',16);
     ...
     if (~isnan(d35_lim) && ~isnan(d35_limhi) && ~isnan(d35_limlo)) &&...
        ...
        set(gca,'FontSize',16);
        %xlabel('D_{05 cc} [Gy]','FontSize',25);
        xlabel('D_{3.5 cc} [Gy]','FontSize',25);
        ylabel('Complication rate observed','FontSize',22);
    ...
    set(gca,'YScale','log');
    lgnd_pvals=legend(h_pvals,structures,'Location','SouthEast');
    set(lgnd_pvals,'FontSize',18);
    ...
    if do_print,
     ...
     cur_fig=figure(i+10000);
    set(gca,'YScale','log');
    ylim([min([0.04 min(min_lr_pvals)*0.75]) 1]);
    lgnd_mm_pvals=legend(h_mm_pvals,structures,'Location','SouthEast');
    set(lgnd_mm_pvals,'FontSize',18);
    
    if do_print,
        set(cur_fig,'Color','w');
        export_fig(cur_fig,...
            [fig_tox_basename,'_mm_lr_eud_pvals'],'-pdf');
        disp(['Saving ',fig_tox_basename,'_mm_lr_eud_pvals.pdf']);
        
    end
    
    %t-stats
     cur_fig=figure(i+100000);hold on;
     plot(-LymanN,repmat(1.96,size(LymanN),1),'r--','LineWidth',2);
     plot(-LymanN,repmat(-1.96,size(LymanN),1),'r--','LineWidth',2);        
                 set(gca,'xminortick','on','yminortick','on');
            set(gca,'box','on');
            set(gca,'fontsize',16);
...
%    ylim([min([0.04 min(min_lr_pvals)*0.75]) 1]);
    lgnd_mm_tstats=legend(h_mm_tstats,structures,'Location','NorthEast');
    set(lgnd_mm_tstats,'FontSize',18);
     grid off;
    ...
    if do_print,
        set(cur_fig,'Color','w');
        export_fig(cur_fig,...
            [fig_tox_basename,'_mm_lr_eud_tvals'],'-pdf');
        disp(['Saving ',fig_tox_basename,'_mm_lr_eud_tvals.pdf']);
        
    end
       % t-values
    cur_fig=figure(i+1000000);hold on;
